const Data = [
    {
        name: 'star wars',
        image :'./image/desktop-wallpaper-killian-eng-s-new-star-wars-poster-star-wars-movie-poster-thumbnail.jpg',
        price: "$12",
        catagory: 'trending'
    },
    {
        name: 'new moon',
        image :'./image/desktop-wallpaper-new-moon-poster-003-posters-and-twilight-new-moon-movies-thumbnail.jpg',
        price: "$12",
        catagory: 'latest'
    },
    {
        name: 'Lots City',
        image :'./image/desktop-wallpaper-new-movie-posters-hollywood-movie-2022-thumbnail.jpg',
        price: "$12",
        catagory: 'action'
    },
    {
        name: 'Encanto',
        image :'./image/desktop-wallpaper-new-poster-of-disney-s-encanto-r-movies-encanto-movie-thumbnail.jpg',
        price: "$12",
        catagory: 'animation'
    },
       {
        name: 'the mist',
        image :'./image/download (1).jpeg',
        price: "$12",
        catagory: 'horror'
    },
    {
        name: 'curse of nun',
        image :'./image/download (2).jpeg',
        price: "$12",
        catagory: 'documentry'
    },
    {
        name: 'Hollywood banker',
        image :'./image/download (3).jpeg',
        price: "$12",
        catagory: 'science finction'
    },
    {
        name: 'Encanto',
        image :'./image/download (4).jpeg',
        price: "$12",
        catagory: 'drama'
    },
    {
        name: 'goblin',
        image :'./image/download (5).jpeg',
        price: "$12",
        catagory: 'drama'
    },
    {
        name: 'stork',
        image :'./image/download (6).jpeg',
        price: "$12",
        catagory: 'animation'
    },
       {
        name: 'the weight is over',
        image :'./image/download (7).jpeg',
        price: "$12",
        catagory: 'animation'
    },
    {
        name: 'the angry bird',
        image :'./image/download (8).jpeg',
        price: "$12",
        catagory: 'animation'
    },
    {
        name: 'the conjuring',
        image :'./image/download (3).jpeg',
        price: "$12",
        catagory: 'mystry'
    },
    {
        name: 'the conjuring',
        image :'./image/download (3).jpeg',
        price: "$12",
        catagory: 'disaster'
    },
    {
        name: 'Encanto',
        image :'./image/download (4).jpeg',
        price: "$12",
        catagory: 'adventure'
    },
    {
        name: 'goblin',
        image :'./image/download (5).jpeg',
        price: "$12",
        catagory: 'thriller'
    },
    {
        name: 'the vanila sky',
        image :'./image/images (1).jpeg',
        price: "$12",
        catagory: 'animation'
    },
    {
        name: 'another earth',
        image :'./image/images (2).jpeg',
        price: "$12",
        catagory: 'mystry'
    },
    {
        name: 'the conjuring',
        image :'./image/images (1).jpeg',
        price: "$12",
        catagory: 'disaster'
    },
    {
        name: 'Encanto',
        image :'./image/images (1).jpeg',
        price: "$12",
        catagory: 'adventure'
    },
    {
        name: 'goblin',
        image :'./image/images (1).jpeg',
        price: "$12",
        catagory: 'thriller'
    },

]